...
// disabled because some scanners consider any calls to CreateRemoteThread to be suspicious
// even though it's simply an official win32 api function.
#if 0
HANDLE WINAPI RetryingCreateRemoteThread(IN HANDLE hProcess, IN LPSECURITY_ATTRIBUTES lpThreadAttributes,
    IN SIZE_T dwStackSize, IN LPTHREAD_START_ROUTINE lpStartAddress, IN LPVOID lpParameter,
    IN DWORD dwCreationFlags, OUT LPDWORD lpThreadId)
{
	HANDLE rv;
	for(int i = 0; i < s_safeMaxIter; i++)
	{
		rv = CreateRemoteThread(hProcess, lpThreadAttributes, dwStackSize,
...
DWORD WINAPI InjectDLLThreadFunc(LPVOID lpParam) 
{
	...
	// method 1: modify the IDT (import directory table) (though the IAT is more commonly referred to)
...
// this was the backup method in case N-InjectLib fails,
// but it's disabled because certain virus scanners (such as F-PROT)
// will flag any exe that calls CreateRemoteThread as infected (at the time of this writing),
// even though it's simply a standard Win32 API function.
// trying to evade that detection seems like a bad idea, so it's simply disabled instead.
// people put way too much trust in these scanners considering their high false-positive rates...
#if 0
	// method 2: write code to call LoadLibrary on our DLL into the game process
	//   then create a remote thread that runs that code.
	//   the disadvantage is that other DLLs might get a chance to run code first
	//   if those DLLs were statically linked with the game

	LPVOID lpRemoteBuffer = RetryingVirtualAllocEx(
		hProcess,
		NULL,
		dwPathLength,
	...
	{
		PrintLastError("VirtualAllocEx", GetLastError());
		terminateRequest = true;
	}
	else
		debugprintf("allocated %d bytes remotely\n", dwPathLength);

	SIZE_T dwWritten;
	if (!RetryingWriteProcessMemory(hProcess, lpRemoteBuffer, dllPath, dwPathLength, &dwWritten))
	{
		PrintLastError("WriteProcessMemory", GetLastError());
		terminateRequest = true;
	}
	else
		debugprintf("%d bytes written to remote process 0x%X\n", dwWritten, hProcess);

	LPTHREAD_START_ROUTINE startup = (LPTHREAD_START_ROUTINE)GetProcAddress(LoadLibrary("kernel32"), "LoadLibraryA");

	debugprintf("Injecting \"%s\" to start with call to 0x%X...\n", dllPath, startup);
	{
		...
		{
			debugprintf("WARNING: \"%s\" doesn't exist!!\n", dllPath);
			terminateRequest = true;
		}
	}

	HANDLE hThread = RetryingCreateRemoteThread(
		hProcess,
		NULL,
		0,
		startup,
	...
	if (hThread == NULL)
	{
		PrintLastError("CreateRemoteThread", GetLastError());
		terminateRequest = true;
	}
	else
		debugprintf("remote thread created (0x%X)\n", dwThread);

	info->injectIsAsyncReady = TRUE;

	WaitForSingleObject(hThread, INFINITE);

	if(!terminateRequest && !GetExitCodeThread(hThread, &exitCode))
		exitCode = 0;

	// cleanup
	VirtualFreeEx(hProcess, lpRemoteBuffer, dwPathLength, MEM_DECOMMIT);
	CloseHandle(hThread);
#else
//	// add NON-system-wide hook
//	HMODULE hHookDll = LoadLibraryA(dllPath);
//	if(hHookDll)
//	{
//		//HOOKPROC hHookProc = (HOOKPROC)GetProcAddress(hHookDll, "nothinghook");
//		HOOKPROC hHookProc = (HOOKPROC)GetProcAddress(hHookDll, "_nothinghook@12"); // stupid function decoration. I could add a .DEF file but that seems overkill for a dll that isn't really exporting any functionality
//		if(hHookProc)
//		{
//			if(dwInjectThreadID)
//			{
//// DOESN'T WORK, apparently it only works on processes that have started already, or maybe only the current process if not global.
//				HHOOK hHook = SetWindowsHookEx(WH_FOREGROUNDIDLE,hHookProc,hHookDll,dwInjectThreadID);
//				if(hHook)
//				{
//					exitCode = 64;
//				}
...
static void SaveMovieToFile(Movie& movie, const char* filename)
{
	...
	FILE* file = (filename && *filename) ? fopen(filename, "wb") : NULL;
	if(!file)
	{
		char str [1024];
	...
	int fpsp1 = max(0,framerate)+1;
	fwrite(&fpsp1, 4, 1, file);

	int itp1 = initialTime+1;
	fwrite(&itp1, 4, 1, file);

	int crc = CalcExeCrc();
	fwrite(&crc, 4, 1, file);

	int fsize = CalcExeFilesize();
...
static int LoadMovieFromFile(/*out*/ Movie& movie, const char* filename, bool forPreview=false, bool secondaryProcessing=false)
{
	...
	else // invalid movie file (also happens whenever starting a new movie file)
	{
		movie.currentFrame = 0;
	...
	if((playback || forPreview) && !secondaryProcessing)
	{
		...
		if(playback && !forPreview && movieInitialTime != initialTime)
		{
			...
			if(result == IDCANCEL)
				return -1;
			if(result == IDYES)
				movieInitialTime = initialTime;
		}
		initialTime = movieInitialTime;
		char str [256];
		sprintf(str, "%u", initialTime);
...
static void* remoteLastFrameSoundInfo = 0;

void ReceiveSoundCaptureInfoPointer(__int64 pointerAsInt)
{
	remoteLastFrameSoundInfo = (void*)pointerAsInt;
}
...
void ReceiveGammaRampData(__int64 pointerAsInt, HANDLE hProcess)
{
	...
	{
		SIZE_T bytesRead = 0;
		if(ReadProcessMemory(hProcess, (void*)pointerAsInt, &g_gammaRamp, sizeof(DDGAMMARAMP), &bytesRead))
		{
...
void ReceiveKeyboardLayout(__int64 pointerAsInt, HANDLE hProcess)
{
	...
	if(playback && *movie.keyboardLayoutName)
...
//#endif
	...
	if(fastforward && !paused && !recoveringNearTargetFrame && goingsuperfast)
	{
	...
	if(FrameAdvanceKeyDown && started)
	{
		...
		else
		{
			...
			else
			{
				...
				const int frameAdvanceSubsequentWait = fastforward ? 20 : 50;
...
void UpdateFrameCountDisplay(int frameCount, int frequency)
{
	...
	if(frameCount % frequency == 0)
	{
		...
		if(maxcount == frameCount-1 && !paused && !playback)
		...
		if(frameCount != displayedFrameCount || maxcount != displayedMaxFrameCount)
		{
			...
			}
			{
				if(framerate > 0)
				{
					float seconds = fmodf((float)frameCount / framerate, 60.0f);
					float maxseconds = fmodf((float)maxcount / framerate, 60.0f);
					int minutes = frameCount / (60 * framerate) % 60;
					int maxminutes = maxcount / (60 * framerate) % 60;
					int hours = frameCount / (60*60 * framerate);
					int maxhours = maxcount / (60*60 * framerate);
...
void CheckDialogChanges(int frameCount)
{
	if(!displayed_checkdialog_inited ||
		...
		playback != displayed_playback ||
		finished != displayed_finished)
	{
		displayed_started = started;
		displayed_playback = playback;
...
//		EnableWindow(GetDlgItem(hWnd, IDC_EMULATESOUND), !started);
		EnableWindow(GetDlgItem(hWnd, IDC_AVIAUDIO), !started || (emuMode & EMUMODE_EMULATESOUND));
	...
	if(!displayed_checkdialog_inited || displayed_fastforward != fastforward)
	{
		displayed_fastforward = fastforward;
		CheckDlgButton(hWnd, IDC_FASTFORWARD, fastforward);
	}
	...
	if(!displayed_checkdialog_inited || displayed_threadMode != threadMode || displayed_waitSyncMode != waitSyncMode)
	{
		displayed_threadMode = threadMode;
		displayed_waitSyncMode = waitSyncMode;
		CheckDlgButton(hWnd, IDC_RADIO_THREAD_DISABLE, threadMode == 0 && waitSyncMode == 1);
		CheckDlgButton(hWnd, IDC_RADIO_THREAD_WRAP, threadMode == 1 && waitSyncMode == 1);
		CheckDlgButton(hWnd, IDC_RADIO_THREAD_ALLOW, threadMode == 2 && waitSyncMode == 2);
	}
	if(displayed_muteMode != ((emuMode & EMUMODE_NOPLAYBUFFERS) ? 1 : 0))
	{
		displayed_muteMode = ((emuMode & EMUMODE_NOPLAYBUFFERS) ? 1 : 0);
...
struct AviFrameQueue
{
                   ...
                   int rmask, int gmask, int bmask)
	{
		...
		if(pitch >= 0)
		{
			...
			ReadProcessMemory(hGameProcess, remotePixels, inPixels, remotePixelsSize, NULL);
			curInPixels = inPixels;
		}
		else
		{
			// we got a pointer to the start of the last row of pixel data (see FrameBoundaryDIBitsToAVI)
			ReadProcessMemory(hGameProcess, (char*)remotePixels + pitch*(height-1), inPixels, remotePixelsSize, NULL);
	...
	void FillAudioFrame()
	{
		...
		ReadProcessMemory(hGameProcess, soundInfo.buffer, slot.audioBuffer, soundInfo.size, NULL);
			...
			//		AutoCritSect cs(&s_aviCS);
...

					{
						...
						{ 
							...
							MessageBox(hWnd, "AVIStreamWrite failed! (Sorry... try restarting this program and/or choosing a different codec.)\n", "Error", MB_OK|MB_ICONERROR);
							CloseAVI();
				...
				if(aviMode & 2)
				{
...

					{
						...
						{ 
							...
							MessageBox(hWnd, "AVIStreamWrite(audio) failed!\n", "Error", MB_OK|MB_ICONERROR);
							CloseAVI();
							//CheckDlgButton(hWnd, IDC_AVIVIDEO, 0);
							//CheckDlgButton(hWnd, IDC_AVIAUDIO, 0);
							aviMode = 0;
...
bool OpenAVIFile(int width, int height, int bpp, int fps)
{
	...
	if(width && height && fps)
	{
		...
		{ 
			...
			MessageBox(hWnd, "AVIFileCreateStream failed!\n", "Error", MB_OK|MB_ICONERROR);
			CloseAVI();
			return false;
		}
...
chooseAnotherFormat:
		...
		{
			if(!AVISaveOptions(hWnd, ICMF_CHOOSE_KEYFRAME|ICMF_CHOOSE_DATARATE, 1, &aviStream, &pOptions))
			{
				CloseAVI();
				return false;
			}
		...
		{ 
			...
			MessageBox(hWnd, "AVIMakeCompressedStream failed!\n", "Error", MB_OK|MB_ICONERROR);
			//CloseAVI();
			//return false;
		...
		{ 
			...
			MessageBox(hWnd, "AVIStreamSetFormat failed!\n", "Error", MB_OK|MB_ICONERROR);
			//CloseAVI();
			//return false;
			AVIStreamClose(aviCompressedStream);
			aviCompressedStream = NULL;
...
void WriteAVIFrame(void* remotePixels, int width, int height, int pitch, int bpp,
                   int rmask, int gmask, int bmask)
{
	if(!(aviMode & 1))
		return; // double-check...
...

	{
		...
		for(int chooseIter = 1;; chooseIter++)
		{
			...
			if(audioConverterStream->failed)
			{
				...
				MessageBox(hWnd, "Couldn't find a valid conversion sequence.\nTry a different audio codec.\n", "Error", MB_OK|MB_ICONERROR);
				delete audioConverterStream;
				audioConverterStream = NULL;
			}
			else
		...
		{ 
			...
			MessageBox(hWnd, "AVIFileCreateStream(audio) failed!\n", "Error", MB_OK|MB_ICONERROR);
			CheckDlgButton(hWnd, IDC_AVIAUDIO, 0);
			aviMode &= ~2;
			tasFlagsDirty = true;
			return -1;
		...
		{ 
			...
			MessageBox(hWnd, "AVIStreamSetFormat(audio) failed!\n", "Error", MB_OK|MB_ICONERROR);
			CheckDlgButton(hWnd, IDC_AVIAUDIO, 0);
			aviMode &= ~2;
			tasFlagsDirty = true;
			return -1;
...
void ProcessCaptureFrameInfo(void* frameCaptureInfoRemoteAddr, int frameCaptureInfoType)
{
	...
	case CAPTUREINFO_TYPE_PREV:
		if((aviMode & 1) && aviCompressedStream)
		{
			aviFrameQueue->RefillFrame();
		}
		break;
	case CAPTUREINFO_TYPE_DDSD:
		if((aviMode & 1) && frameCaptureInfoRemoteAddr)
		{
			DDSURFACEDESC desc = {0};
			ReadProcessMemory(hGameProcess, frameCaptureInfoRemoteAddr, &desc, sizeof(desc), NULL);
...
void FrameBoundary(const char* frameInfo, int threadId)
{
	...
	if(!requestedCommandReenter)
	{
		...
		int frameCountUpdateFreq = framerate/2;
		if(fastforward && !(aviMode & 1))
...
#pragma region OUTPUT_DEBUG_STRING_EVENT
					...
					{
						SIZE_T bytesRead = 0;
						if(!ReadProcessMemory(hProcess, dsi.lpDebugStringData, str, len, &bytesRead))
		...
		case WM_SHOWWINDOW:
 			{
				...
				initialTime = /*timeGetTime()*/6000;
				sprintf(str, "%d", initialTime);
...
//				CheckDlgButton(hDlg, IDC_EMULATESOUND, emuMode & EMUMODE_EMULATESOUND);
				...
				CheckDlgButton(hDlg, IDC_RADIO_THREAD_DISABLE, threadMode == 0);
				CheckDlgButton(hDlg, IDC_RADIO_THREAD_WRAP, threadMode == 1);
				CheckDlgButton(hDlg, IDC_RADIO_THREAD_ALLOW, threadMode == 2);
		...
		case WM_COMMAND:
			{
				...
				case ID_GRAPHICS_MEM_SYSTEM:
					forceSurfaceMemory = 1;
					tasFlagsDirty = true;
					break;
				case ID_GRAPHICS_MEM_NONLOCAL:
					forceSurfaceMemory = 2;
					tasFlagsDirty = true;
					break;
				case ID_GRAPHICS_MEM_LOCAL:
					forceSurfaceMemory = 3;
					tasFlagsDirty = true;
					break;
				...
				case ID_SOUND_RATE_8000:  audioFrequency = 8000;  tasFlagsDirty = true; break;
				case ID_SOUND_RATE_11025: audioFrequency = 11025; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_12000: audioFrequency = 12000; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_16000: audioFrequency = 16000; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_22050: audioFrequency = 22050; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_24000: audioFrequency = 24000; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_32000: audioFrequency = 32000; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_44100: audioFrequency = 44100; tasFlagsDirty = true; break;
				case ID_SOUND_RATE_48000: audioFrequency = 48000; tasFlagsDirty = true; break;
				case ID_SOUND_BITS_8:  audioBitsPerSecond = 8;  tasFlagsDirty = true; break;
				case ID_SOUND_BITS_16: audioBitsPerSecond = 16; tasFlagsDirty = true; break;
				case ID_SOUND_CHANNELS_1: audioChannels = 1; tasFlagsDirty = true; break;
				case ID_SOUND_CHANNELS_2: audioChannels = 2; tasFlagsDirty = true; break;
				...
				case IDC_MAKETOPMOST:
					windowActivateFlags = (windowActivateFlags & ~2) | (IsDlgButtonChecked(hDlg, IDC_MAKETOPMOST) ? 2 : 0);
					tasFlagsDirty = true;
					break;

				case ID_TIME_FF_FRONTSKIP:
					fastForwardFlags ^= FFMODE_FRONTSKIP;
					tasFlagsDirty = true;
					break;
				case ID_TIME_FF_BACKSKIP:
					fastForwardFlags ^= FFMODE_BACKSKIP;
					tasFlagsDirty = true;
					break;
				case ID_TIME_FF_SOUNDSKIP:
					fastForwardFlags ^= FFMODE_SOUNDSKIP;
					tasFlagsDirty = true;
					break;
				case ID_TIME_FF_RAMSKIP:
					fastForwardFlags ^= FFMODE_RAMSKIP;
					tasFlagsDirty = true;
					break;
				case ID_TIME_FF_SLEEPSKIP:
					fastForwardFlags ^= FFMODE_SLEEPSKIP;
					tasFlagsDirty = true;
					break;
				case ID_TIME_FF_WAITSKIP:
					fastForwardFlags ^= FFMODE_WAITSKIP;
					tasFlagsDirty = true;
					break;
				...
				case ID_TIME_RATE_SLOWER:
					if(timescale==timescaleDivisor) {timescale = 3; timescaleDivisor = 4; tasFlagsDirty = true;}
					else if(timescale*4==timescaleDivisor*3) {timescale = 1; timescaleDivisor = 2; tasFlagsDirty = true;}
					else if(timescale*2==timescaleDivisor) {timescale = 1; timescaleDivisor = 4; tasFlagsDirty = true;}
					else if(timescale*4==timescaleDivisor) {timescale = 1; timescaleDivisor = 8; tasFlagsDirty = true;}
					break;
				case ID_TIME_RATE_FASTER:
					if(timescale*4==timescaleDivisor*3) {timescale = 1; timescaleDivisor = 1; tasFlagsDirty = true;}
					else if(timescale*2==timescaleDivisor) {timescale = 3; timescaleDivisor = 4; tasFlagsDirty = true;}
					else if(timescale*4==timescaleDivisor) {timescale = 1; timescaleDivisor = 2; tasFlagsDirty = true;}
					else if(timescale*8==timescaleDivisor) {timescale = 1; timescaleDivisor = 4; tasFlagsDirty = true;}
					break;
				...
				case ID_EXEC_DLLS_INSTALLED:
					allowLoadInstalledDlls = !allowLoadInstalledDlls;
					tasFlagsDirty = true;
					break;
				...
				case IDC_RADIO_THREAD_DISABLE:
					threadMode = 0;
					waitSyncMode = 1; // hack, should rename these radio buttons
					tasFlagsDirty = true;
					break;
				...
				case IDC_RADIO_THREAD_ALLOW:
					threadMode = 2;
					waitSyncMode = 2; // hack, should rename these radio buttons
					tasFlagsDirty = true;
					break;
				case IDC_EDIT_FPS:
					{
					...
					if(!started && !unsaved)
					{
						if(HIWORD(wParam) == EN_CHANGE)
						{
							...
							if(!selfEdit)
							{
								...
								initialTime = strtoul(str,0,0);
								if(!*str) initialTime = /*timeGetTime()*/6000;
								//char str2 [256];
		...
		case WM_CTLCOLORSTATIC:
			...
			&& started) 
			{
				...
				else if(playback) // turn text blue when movie is playing
				{
					HDC hdcStatic = (HDC)wParam;
